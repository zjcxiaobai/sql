...
ELSE  'bulk'
END AS prod_qty_type_condensed,
CASE WHEN product_name LIKE '%pepper%'  COLLATE NOCASE THEN 1  
ELSE 0
END AS pepper_flag
FROM product;
...
COUNT(vba.vendor_id) AS rental_count
FROM vendor AS v
INNER JOIN vendor_booth_assignments AS vba
  ON v.vendor_id = vba.vendor_id
...
*/
-- delete exsiting temp table if exist
...
-- check if I successfully insert the VALUES
SELECT *
...
and year are! */
SELECT customer_id,
CAST(strftime('%m', market_date) AS INTEGER) AS month,  -- 1..12
CAST(strftime('%Y', market_date) AS INTEGER) AS year    -- e.g. 2025
FROM customer_purchases;
...
SELECT c.customer_id, c.customer_first_name, c.customer_last_name,
SUM(cp.quantity * cp.cost_to_customer_per_qty) AS total_spent_apr_2022
